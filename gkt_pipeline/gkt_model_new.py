...
import logging
from data_processing import create_result_df_all_mean
import pandas as pd
...
def initialize_gkt_model(chunjae_math, label_math_ele_12, result_df_all):
    logging.info("천재교육_계열화 데이터와 개념 관계 데이터를 매핑하여 신규 학습자의 GKT 모델을 초기화합니다.")
    
    # 'knowledgeTag'와 '중단원' 매핑
    ...
    # 평균 데이터 생성
    result_df_all_mean = create_result_df_all_mean(result_df_all)
    
    # 지식 상태 업데이트
    for idx, row in result_df_all_mean.iterrows():
        ...
        # Learning_state 설정
        mean_Learning_state = row['mean_Learning_state']
        Learning_state_counts = {state: 0 for state in weights.keys()}
        Learning_state_counts[mean_Learning_state] = 1  # 설정된 상태에 1 할당
...
def visualize_gkt_model_3d(model, chunjae_math, title='GKT Model 3D Visualization'):
    logging.info("전체 학습자 평균 GKT 모델을 시각화합니다.")
...
# 신규 학습자 보고서 생성
def generate_new_report(model):
    logging.info("보고서를 생성합니다.")
...
class EnhancedGKTModelDetailed(EnhancedGKTModel):
    def update_knowledge(self, knowledge_element, Learning_state_counts):
        ...
        Learning_state_counts = {state: Learning_state_counts.get(state, 0) for state in weights.keys()}
        total_counts = sum(Learning_state_counts.values())
        weighted_score = sum(Learning_state_counts[state] * weights[state] for state in weights.keys()) / total_counts if total_counts > 0 else 0
        self.weighted_scores[knowledge_element] = weighted_score
...
# 세분화된 그래프를 위한 모델 생성 함수
def create_detailed_gkt_model(model):
    logging.info("세분화된 전체 학습자 평균 GKT 모델을 생성합니다.")
    
    # red 상태 노드 추출 및 서브그래프 생성
    red_nodes = [node for node, state in model.knowledge_state.items() if state == 'red']
    subgraph = model.graph.subgraph(red_nodes).copy()

    # EnhancedGKTModelDetailed 인스턴스 생성
    detailed_model = EnhancedGKTModelDetailed(subgraph.edges(), subgraph.nodes())

    # 기존 모델의 weighted_score을 바탕으로 detailed 모델의 지식 상태 업데이트
    for node in subgraph.nodes():
        weighted_score = model.weighted_scores.get(node, 0)
        detailed_model.weighted_scores[node] = weighted_score
        if weighted_score >= 0.7:
            detailed_model.knowledge_state[node] = 'yellow'
        elif weighted_score >= 0.5:
            detailed_model.knowledge_state[node] = 'orange'
        else:
            detailed_model.knowledge_state[node] = 'red'
    
    return detailed_model

# 세분화된 GKT 모델 시각화 함수
def visualize_gkt_model_3d_detailed(model, title='GKT Model 3D Visualization (선수학습이 필요한 노드의 Learning_state)'):
    logging.info("전체 학습자 평균 상세 GKT 모델을 시각화합니다.")
    pos = nx.spring_layout(model.graph, dim=3, k=0.7, seed=42)
